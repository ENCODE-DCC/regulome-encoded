    ...
    filesToTracks(files, domain) {
        const tracks = files.map((file) => {
            if (file.name) {
                ...
                const trackObj = {};
                trackObj.name = `${(file.target && file.target.label) ? ` ${file.target.label} ` : ''}${file.assay_term_name} - ${(file.biosample_ontology && file.biosample_ontology.term_name) ? file.biosample_ontology.term_name : ''}`;
                trackObj.type = 'signal';
