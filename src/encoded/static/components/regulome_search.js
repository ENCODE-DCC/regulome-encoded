...
const fieldsToSave =
  "&field=cloud_metadata.url&field=title&field=href&field=path&field=file_format_type&field=dataset&field=biosample_ontology.term_name&field=target&field=file_format&field=biosample_ontology.organ_slims&field=biosample_ontology.cell_slims&field=assay_term_name&field=biological_replicates";
...
const dataTypeStrings = [
  {
    type: "dbSNP IDs",
    explanation:
      "Enter dbSNP IDs or upload a list of dbSNP IDs to identify DNA features and regulatory elements that contain the coordinate of the SNPs.",
  },
  {
    type: "Single nucleotides",
    explanation:
      "Enter hg19 coordinates for a single nucleotide as 0-based. These coordinates will be mapped to dbSNP IDs (if available) in addition to identifying DNA features and regulatory elements that contain the input coordinates.",
  },
  {
    type: "A chromosomal region",
    explanation:
      "Enter hg19 chromosomal regions, such as a promoter region upstream of a gene, as 0-based coordinates. All dbSNP IDs with an minor allele frequency >1% that are found in this region will be used to identify DNA features and regulatory elements that contain the coordinate of the SNPs.",
  },
];
...
const dataColumnsChromatin = {
  ...
  biosample_term_name: {
    title: "Biosample",
    getValue: (item) =>
      item.biosample_ontology ? item.biosample_ontology.term_name : "",
  },
  biosample_classification: {
    title: "Classification",
    getValue: (item) =>
      item.biosample_ontology ? item.biosample_ontology.classification : "",
  },
  organ_slims: {
    title: "Organ",
    getValue: (item) =>
      item.biosample_ontology &&
      item.biosample_ontology.organ_slims &&
      item.biosample_ontology.organ_slims.length > 0
        ? item.biosample_ontology.organ_slims.join(", ")
        : "",
  },
  dataset: {
    title: "Dataset",
  ...
  file: {
    title: "File",
    display: (item) => (
      <a href={`https://encodeproject.org/files/${item.file}/`}>{item.file}</a>
    ),
  },
};
...
const dataColumnsChip = {
  ...
  targets: {
    title: "Targets",
    getValue: (item) =>
      item.targets && item.targets.length > 0 ? item.targets.join(", ") : "",
  },
  organ_slims: {
    title: "Organ",
    getValue: (item) =>
      ...
      <React.Fragment>
        <h4>
          ...
          >
            <i
              className={`icon ${
                this.state.open ? " icon-caret-down" : "icon-caret-right"
              }`}
            />{" "}
            {this.props.type}
          </div>
        </h4>
        <p className={`data-type-explanation${this.state.open ? " show" : ""}`}>
...
class ExampleEntry extends React.Component {
  ...
  render() {
    return (
      <button className="example-input" onClick={this.handleClick}>
        {" "}
        {this.props.label}{" "}
      </button>
    );
  ...
  return (
    <div className="assembly-switch">
      ...
      <label
        ...
        aria-label={`Assembly is ${props.selection}. Toggle to switch to ${
          props.selection === "GRCh38" ? "hg19" : "GRCh38"
        }`}
      >
        <input
          type="checkbox"
          ...
          onChange={() => {
            props.onSelection(props.selection);
          }}
          onKeyDown={(e) => {
            props.onSelection(props.selection, e);
          }}
      ...
      <div
        className={`switch-label ${props.selection === "hg19" ? "active" : ""}`}
      >
        hg19
      </div>
    </div>
...
class AdvSearch extends React.Component {
  ...
  handleOnFocus() {
    const summaryHref = `/regulome-summary/?regions=${this.state.searchInput.replaceAll(
      "\n",
      "%0D%0A"
    )}&genome=${this.state.genome}&maf=${this.state.maf}`;
    if (this.state.searchInput) {
      fetchData(summaryHref, this.context.fetch).then((response) => {
        if (response.total > 0) {
          ...
          this.setState({
            modal: "Please define valid SNP(s) to complete a search.",
          });
        }
      });
    } else {
      this.setState({
        modal: "Please define at least one SNP to complete a search.",
      });
    }
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <div
          id="panel1"
          className="adv-search-form"
          autoComplete="off"
          aria-labelledby="tab1"
          onSubmit={this.handleOnFocus}
        >
          <div className="form-group">
            <label htmlFor="annotation">
              <i className="icon icon-search" />
              Search by dbSNP ID or coordinate range:{" "}
              <AssemblySelector
            ...
            <div className="input-group input-group-region-input">
              <textarea
                className="multiple-entry-input"
                id="multiple-entry-input"
              ...
              <div className="example-inputs">
                ...
                {exampleEntries.map((entry, entryIdx) => (
                  <span key={entry.label}>
                    ...
                    />
                    {entryIdx !== exampleEntries.length - 1 ? "or" : null}
                  </span>
                ))}
              </div>
        ...
        context.notification !== "No annotations found" ? (
        ...
        context.regulome_score.probability &&
        context.regulome_score.ranking ? (
          <p className="regulomescore">
            RegulomeDB score: {context.regulome_score.probability}{" "}
            (probability); {context.regulome_score.ranking} (ranking){" "}
          </p>
        ) : null}
        ...
        {context.peak_details !== undefined && context.peak_details !== null ? (
          <div className="btn-container">
            <a
              className="btn btn-info btn-sm"
              href={context.download_elements[0]}
              data-bypass
            >
              Download peak details (TSV)
            </a>
            <a
              className="btn btn-info btn-sm"
              href={context.download_elements[1]}
              data-bypass
            >
              Download peak details (JSON)
            </a>
          </div>
        ) : null}
      </React.Fragment>
    ...
    <React.Fragment>
      {data.length > 0 ? (
        ...
        <table className="table table-sortable table-panel">
          {displayTitle ? (
            <tr className="table-section" key="title">
              <th colSpan={colCount}>{displayTitle}</th>
            </tr>
          ) : null}
          <thead>
            <tr key="header">
              {Object.keys(dataColumns).map((columnId) => (
                <th key={columnId}>{dataColumns[columnId].title}</th>
              ))}
            </tr>
          </thead>
          <tbody>
...
const chunkingDataset = (
  ...
  const promiseList = [];
  for (
    let chunkIdx = startIdxWrapper;
    chunkIdx < endIdxWrapper;
    chunkIdx += 1
  ) {
...
export class RegulomeSearch extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState, nextContext) {
    ...
    const screenSizeUpdate =
      this.state.screenWidth !== this.applicationRef.offsetWidth;
    const pageUpdate =
      this.state.browserCurrentPage !== nextState.browserCurrentPage;
    const filtersUpdate =
      this.state.selectedFilters !== nextState.selectedFilters;
    const facetDisplayUpdate =
      this.state.facetDisplay !== nextState.facetDisplay;
    const paginationChange =
      this.state.multipleBrowserPages !== nextState.multipleBrowserPages;
    const showFreqsToggled =
      this.state.showMoreFreqs !== nextState.showMoreFreqs;
    const genomeUpdate = this.state.genome !== nextState.genome;
    const filesUpdate =
      this.state.includedFiles.length !== nextState.includedFiles.length;
    const updateBool =
  ...
  handleFacetList(selectedFilters) {
    const filteredFiles = filterByAllSelectedFilters(
      this.state.allFiles,
      selectedFilters,
      facetParameters
    );
  ...
  loadHg19ValisData() {
    if (this.state.filteredFiles.length < 1 && this.props.context["@graph"]) {
      ...
      const duplicatedExperimentDatasets = this.props.context["@graph"].filter(
        (d) => d.dataset.includes("experiment")
      );
      ...
      const chipDatasets = experimentDatasets.filter(
        (d) => d.method === "ChIP-seq"
      );
      const dnaseDatasets = experimentDatasets.filter(
        (d) => d.method === "DNase-seq"
      );
      const faireDatasets = experimentDatasets.filter(
        (d) => d.method === "FAIRE-seq"
      );
      // how many queries we need to run based on number of datasets per query
      const numChipChunks = Math.ceil(
        Object.keys(chipDatasets).length / chunkSize
      );
      const numDnaseChunks = Math.ceil(
        Object.keys(dnaseDatasets).length / chunkSize
      );
      const numFaireChunks = Math.ceil(
        Object.keys(faireDatasets).length / chunkSize
      );
      ...
      const chipPromises = chunkingDataset(
        requests,
        0,
        numChipChunks,
        chipDatasets,
      ...
      const dnasePromises = chunkingDataset(
        requests,
        0,
        numDnaseChunks,
        dnaseDatasets,
      ...
      const fairePromises = chunkingDataset(
        requests,
        0,
        numFaireChunks,
        faireDatasets,
      ...
      Promise.all(allPromises).then((results) => {
        this.setState({ allFiles: results.flat() }, () => {
          ...
          const trimmedFiles0 = sortedFiles.filter((file) => {
            const DatasetFiles0 = sortedFiles.filter(
              (f2) => f2.dataset === file.dataset
            );
          ...
          const trimmedFiles = trimmedFiles0.filter((file) => {
            const datasetFiles = trimmedFiles0.filter(
              (f2) => f2.dataset === file.dataset
            );
          ...
          if (trimmedFiles.length > displaySize) {
            ...
            const browserTotalPages = Math.ceil(
              trimmedFiles.length / displaySize
            );
            this.setState({
      ...
      Promise.all(allPromises).then((results) => {
        this.setState({ allFiles: results.flat() }, () => {
          ...
          if (sortedFiles.length > displaySize) {
            ...
            const browserTotalPages = Math.ceil(
              sortedFiles.length / displaySize
            );
            this.setState({
  ...
  render() {
    ...
    const dnaseData = allData.filter(
      (d) => d.method === "FAIRE-seq" || d.method === "DNase-seq"
    );
    const chromatinData = allData.filter((d) => d.method === "chromatin state");
    const thumbnail =
      this.context.location_href.split("/thumbnail=")[1] || null;

    const toggleFreqsShow = () =>
      this.setState((state) => ({ showMoreFreqs: !state.showMoreFreqs }));
    const hitSnps = {};
          ...
          if (snp.ref_allele_freq) {
            ...
            if (Object.keys(snp.ref_allele_freq[refAlleleTag]).length !== 0) {
              Object.keys(snp.ref_allele_freq).forEach((allele) => {
                Object.keys(snp.ref_allele_freq[allele]).forEach(
                  (population) => {
              ...
              Object.keys(snp.alt_allele_freq).forEach((allele) => {
                Object.keys(snp.alt_allele_freq[allele]).forEach(
                  (population) => {
    ...
    return (
      ...
      >
        ...
        {Object.keys(this.props.context.notifications)[0] === "Failed" &&
        context.total !== 0 ? (
          <React.Fragment>
            {Object.keys(this.props.context.notifications).map(
              (note, noteIdx) => (
                <div className="notification-line wider" key={noteIdx}>
                  <span className="notification-label">{note}</span>
                  <span className="notification">
                    {this.props.context.notifications[note]}
                  </span>
                </div>
              )
            )}
          ...
          <React.Fragment>
            <div className="lead-logo">
              <a href="/">
                <img
                  src="/static/img/RegulomeLogoFinal.gif"
                  alt="Regulome logo"
                />
              ...
              <div className="search-information">
                ...
                {allData && allData.length > 0 ? (
                  <div className="notification-line">
                    ...
                    <div className="notification">
                      {allData.length - chromatinData.length} peaks
                    </div>
                  </div>
                ) : null}
                {context.regulome_score ? (
                  ...
                  <div className="notification-line" key={rsid}>
                    ...
                    <div className="notification">
                      ...
                      {sortedPopulations[rsid].length > defaultDisplayCount ? (
                        <button onClick={toggleFreqsShow}>
                          {sortedPopulations[rsid].length - 3}{" "}
                          {this.state.showMoreFreqs ? "fewer" : "more"}
                        </button>
                      ) : null}
                    </div>
                  </div>
              ...
              >
                ...
                >
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        Click here to view tracks in a genome browser.
                        <div>
                          (<b>{this.state.filteredFiles.length}</b> track
                          {this.state.filteredFiles.length !== 1 ? "s" : ""})
                        </div>
                      </div>
                      {this.state.filteredFiles.length > 0 ? (
                        <div className="image-container">
                          <img
                            src={`/static/img/browser-thumbnail-${this.state.genome.toLowerCase()}.png`}
                            alt="Click to view the genome browser"
                          />
                        </div>
                      ) : null}
                    </React.Fragment>
                ...
                >
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        Click to see detailed ChIP-seq data.
                        <div>
                          (<b>{chipData.length}</b> result
                          {chipData.length !== 1 ? "s" : ""})
                        </div>
                      </div>
                      {chipData.length > 0 ? (
                        <BarChart
                          data={chipData}
                          dataFilter={"chip"}
                          chartWidth={this.state.thumbnailWidth}
                          chartLimit={10}
                ...
                >
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        Click to view chromatin data.
                        <div>
                          (<b>{chromatinData.length}</b> result
                          {chromatinData.length !== 1 ? "s" : ""})
                        </div>
                      </div>
                      {chromatinData.length > 0 ? (
                        <BarChart
                          data={chromatinData}
                          dataFilter={"chromatin"}
                          chartWidth={this.state.thumbnailWidth}
                          chartLimit={10}
                          chartOrientation={"horizontal"}
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        {`Click to see ${
                          this.state.genome === "hg19" ? "FAIRE-seq or" : ""
                        } DNase-seq experiments.`}
                        <div>
                          (<b>{dnaseData.length}</b> result
                          {dnaseData.length !== 1 ? "s" : ""})
                        </div>
                      </div>
                      {dnaseData.length > 0 ? (
                        <BarChart
                ...
                >
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        Click to see PWM and Footprint data.
                      </div>
                ...
                >
                  ...
                  {thumbnail === null ? (
                    <React.Fragment>
                      <div className="line">
                        <i className="icon icon-chevron-circle-right" />
                        Click to see dsQTL and eQTL data.
                        <div>
                          (<b>{QTLData.length}</b> result
                          {QTLData.length !== 1 ? "s" : ""})
                        </div>
                      </div>
                      <ResultsTable
                        ...
                        dataFilter={"qtl"}
                        errorMessage={
                          "No result table is available for this SNP."
                        }
                        shortened
                      />
                    </React.Fragment>
                ...
                <React.Fragment>
                  {thumbnail === "motifs" ? (
                    ...
                    <React.Fragment>
                      ...
                      <h4>
                        There{" "}
                        {this.state.filteredFiles.length === 1 ? "is" : "are"}{" "}
                        {this.state.filteredFiles.length} track
                        {this.state.filteredFiles.length === 1 ? "" : "s"}.
                      </h4>
                      <FacetList
                      ...
                      {this.state.multipleBrowserPages ? (
                        <div className="pagination-container">
                          <div>
                            <button
                              disabled={this.state.browserCurrentPage === 1}
                              className="btn btn-page btn-page-left"
                              onClick={() => this.handlePagination("minus")}
                            >
                              <i className="icon icon-chevron-left" />
                            </button>
                            <button
                              disabled={
                                this.state.browserCurrentPage ===
                                this.state.browserTotalPages
                              }
                              className="btn btn-page"
                              onClick={() => this.handlePagination("plus")}
                            >
                              <i className="icon icon-chevron-right" />
                            </button>
                          </div>
                          <div>
                            Page <b>{this.state.browserCurrentPage}</b> of{" "}
                            <b>{this.state.browserTotalPages}</b>
                          </div>
                        </div>
                      ) : null}
                    </React.Fragment>
                      ...
                      {chipData.length > 0 ? (
                        ...
                        <React.Fragment>
                          ...
                          <div className="error-message">
                            No results available to display, please choose a
                            different SNP.
                          </div>
                        </React.Fragment>
                      )}
                    ...
                    <React.Fragment>
                      {dnaseData.length > 0 ? (
                        <ChartList
                          data={dnaseData}
                          displayTitle={"DNA accessibility experiments"}
                          chartWidth={Math.min(this.state.screenWidth, 1000)}
                          dataFilter={thumbnail}
                        />
                      ) : (
                        <React.Fragment>
                    ...
                    <ResultsTable
                      data={QTLData}
                      displayTitle={"dsQTL and eQTL data"}
                      dataFilter={thumbnail}
                      errorMessage={
                        "No result table is available for this SNP."
                      }
          ...
          <React.Fragment>
            ...
            <div className="data-types">
              <div className="data-types-instructions">
                <h4>
                  Use RegulomeDB to identify DNA features and regulatory
                  elements in non-coding regions of the human genome by entering
                  ...
                </h4>
              </div>
            ...
            <div className="link-to-legacy-regulome">
              Looking for the old version? Click{" "}
              <a href="http://legacy.regulomedb.org">here.</a>
            </div>
          </React.Fragment>
        )}
      </div>
