...
export function requestSearch(query) {
    return fetch(`https://encodeproject.org/search/?${query}`, {
        method: 'GET',
        headers: {
...
export function requestFiles(fileIds, filteringFiles) {
    return requestObjects(fileIds, 'https://encodeproject.org/search/?type=File&limit=all&status!=deleted&status!=revoked&status!=replaced', filteringFiles);
}
...
export const InternalTags = ({ context, css }) => {
    const tagBadges = context.internal_tags.map((tag) => {
        const tagSearchUrl = `https://encodeproject.org/search/?type=${context['@type'][0]}&internal_tags=${globals.encodedURIComponent(tag)}&status=released`;
